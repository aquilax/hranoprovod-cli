...
type nodeMap map[string]*node.ParserNode

// Push adds node to the node list
func (db nodeMap) push(pn *node.ParserNode) {
	db[(*pn).Header] = pn
}
...
`
			...
			assert.Nil(t, err)
			nm := (nodeMap)["2011/07/17"]
			assert.Equal(t, "2011/07/17", nm.Header)
			...
			assert.Equal(t, node.Metadata{
				node.MetadataPair{Name: "meta", Value: "value1"},
				node.MetadataPair{Name: "meta", Value: "value2"},
				node.MetadataPair{Name: "", Value: "metadata-no-name"},
			}, *nm.Metadata)
		})
...
func FuzzParseStreamCallback(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, s string) {
		...
		err := ParseStreamCallback(r, NewDefaultConfig(), func(n *node.ParserNode, err error) (bool, error) {
			return false, nil
		})
