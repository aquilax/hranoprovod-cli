...
// Deprecated: Deprecated in favor of using Resolve function directly
func NewResolver(db node.DBNodeMap, c Config) Resolver {
...
func resolveNode(maxDepth int, db node.DBNodeMap, name string, level int) error {
	if level >= maxDepth {
		return fmt.Errorf("maximum resolution depth reached")
	}
...
func Resolve(c Config, db node.DBNodeMap) (node.DBNodeMap, error) {
	for name := range db {
