...
func NewLogNode(time time.Time, elements element.Elements, metadata *Metadata) *LogNode {
	return &LogNode{time, elements, metadata}
}

// NewLogNodeFromElements creates new LogNode from ParserNode elements and time
func NewLogNodeFromElements(time time.Time, elements element.Elements, metadata *Metadata) (*LogNode, error) {
	elList := element.NewElements()
