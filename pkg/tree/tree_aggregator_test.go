	...
	tests := []struct {
	...
	}{
		{
			...
			func() *TreeNode {
				...
				tn.AddDeep(element.NewElement("one/two", 10), DefaultCategorySeparator)
				return tn
			},
			&TreeNode{
				Name:  "root",
		...
		},
		{
			...
			func() *TreeNode {
				tn := NewTreeNode("root", 0)
				tn.AddDeep(element.NewElement("one", 10), DefaultCategorySeparator)
				tn.AddDeep(element.NewElement("one/two", 20), DefaultCategorySeparator)
				return tn
			},
			&TreeNode{
				Name:  "root",
