...
func CSVLog(logStream io.Reader, c CSVLogConfig) error {
	...
	if err := utils.WalkNodesInStream(logStream, c.DateFormat, c.ParserConfig, f, r); err != nil {
		return err
	}
	return r.Flush()
}
...
func CSVDatabase(dbStream io.Reader, cdc CSVDatabaseConfig) error {
	...
	if err := parser.ParseStreamCallback(dbStream, cdc.ParserConfig, func(n *node.ParserNode, err error) (stop bool, cbError error) {
		if err := r.Process(node.NewDBNodeFromNode(n)); err != nil {
			return true, err
		}
		return false, nil
	}); err != nil {
