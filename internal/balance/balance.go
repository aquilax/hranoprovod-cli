...
func Balance(logStream, dbStream io.Reader, bc BalanceConfig) error {
	return utils.WithResolvedDatabase(dbStream, bc.ParserConfig, bc.ResolverConfig,
		func(nl node.DBNodeMap) error {
			...
			if err := utils.WalkNodesInStream(logStream, bc.DateFormat, bc.ParserConfig, f, r); err != nil {
				return err
			}
			return r.Flush()
		})
...
func getReporter(config reporter.Config, db node.DBNodeMap) reporter.Reporter {
