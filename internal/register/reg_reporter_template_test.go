...
func TestRegReporterTemplate(t *testing.T) {
	t.Run("Given template reg reporter", func(t *testing.T) {
		...
		dbElements := element.NewElements()
		dbElements.Add("el1", 1.1)
		dbElements.Add("el2", 1.2)
		dbElements.Add("el3", 1.3)
		dbNode := node.DBNode{Header: "test2", Elements: dbElements}
		db.Push(&dbNode)
		...
		t.Run("Prints list of unresolved items", func(t *testing.T) {
			...
			el := element.NewElements()
			el.Add("test1", 3.1)
			el.Add("test1", 3.1)
			el.Add("test2", 3.2)
			el.Add("test3/test3/test3/test3/test3/test3/test3/test3/test3", 3.3)
			ln := node.NewLogNode(time.Date(2019, 10, 10, 0, 0, 0, 0, time.UTC), el, nil)
			err := rp.Process(ln)
			assert.NoError(t, rp.Flush())
			assert.Nil(t, err)
