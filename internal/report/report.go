	...
	for name, node := range nl {
		for _, el := range node.Elements {
			if el.Name == rec.ElementName {
				list = append(list, element.NewElement(name, el.Value))
			}
		}
	}
	if rec.Descending {
...
func ReportUnresolved(logStream, dbStream io.Reader, ruc ReportUnresolvedConfig) error {
	return utils.WithResolvedDatabase(
		dbStream,
		ruc.ParserConfig,
		ruc.ResolverConfig,
		func(nl node.DBNodeMap) error {
			...
			if err := utils.WalkNodesInStream(logStream, ruc.DateFormat, ruc.ParserConfig, f, r); err != nil {
				return err
			}
			return r.Flush()
		})
...
func ReportQuantity(logStream io.Reader, rqc ReportQuantityConfig) error {
	...
	if err := utils.WalkNodesInStream(logStream, rqc.DateFormat, rqc.ParserConfig, f, r); err != nil {
		return err
	}
	return r.Flush()
}
