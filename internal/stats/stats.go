...
func Stats(logFileName, dbFileName string, sc StatsConfig) error {
	...
	countLog := 0
	if err = parser.ParseFileCallback(logFileName, sc.ParserConfig, func(n *node.ParserNode, _ error) (stop bool, cbError error) {
	...
	countDb := 0
	if err = parser.ParseFileCallback(dbFileName, sc.ParserConfig, func(n *node.ParserNode, _ error) (stop bool, cbError error) {
		countDb++
		return false, nil
	}); err != nil {
		return err
